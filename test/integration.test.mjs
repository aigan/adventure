...
describe('Integration', () => {
  ...
  describe('Complex Scenarios from world.mjs', () => {
    ...
    it.skip('P1.2: trait override order when bases have overlapping knowledge', () => {
      // Tests UnionState override semantics (last base wins)
      // When both bases have knowledge about same subject, last base in array wins
      ...
      world_state.add_beliefs_from_template({
        ...
        workshop: {
          bases: ['Location'],
          traits: {location: 'village', tools: ['hammer', 'anvil']}
        }
      });

      // Create Villager prototype - knows workshop location only
      world_state.add_shared_from_template({
        Villager: {
          bases: ['Person'],
          traits: {
      ...
      // Create Blacksmith prototype - knows workshop location AND tools
      world_state.add_shared_from_template({
        Blacksmith: {
          bases: ['Person'],
          traits: {
      ...
      // Create VillageBlacksmith with BOTH bases
      // Blacksmith is last, so should override Villager's knowledge
      const village_blacksmith = Belief.from_template(world_state, {
        bases: ['Villager', 'Blacksmith'],
        traits: {
          '@label': 'village_blacksmith'
        }
      ...
      const vb = world_state.get_belief_by_label('village_blacksmith');
      const vb_mind = vb.get_trait(world_state, 'mind');
      const vb_state = vb_mind.origin_state;

      // Find the knowledge about workshop
      const workshop = world_state.get_belief_by_label('workshop');
      const workshop_beliefs = [...vb_state.get_beliefs()].filter(b => {
        const about = b.get_about(vb_state);
        return about && about.subject === workshop.subject;
      });

      // Should have exactly ONE belief about workshop (no duplication)
      expect(workshop_beliefs.length).to.equal(1, 'Should have exactly one belief about workshop (merged, not duplicated)');

      // The belief should have BOTH traits (Blacksmith's fuller knowledge)
      const vb_workshop = workshop_beliefs[0];
      expect(vb_workshop.get_trait(vb_state, 'location')).to.not.be.null;
      expect(vb_workshop.get_trait(vb_state, 'tools')).to.not.be.null;

      // Verify tools array has values (Blacksmith's knowledge includes tools)
      const tools = vb_workshop.get_trait(vb_state, 'tools');
      expect(tools).to.be.an('array');
      expect(tools.length).to.be.at.least(1);
    });
