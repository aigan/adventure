...
describe('learn_about', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('learn_about on versioned belief walks chain to find archetypes', () => {
      ...
      const hammer_v1 = world_mind_state.get_belief_by_label('hammer_v1');
    ...
    it('learned belief can be used as trait reference', () => {
      ...
      const workshop_knowledge = learn_about(player_mind_state,
        world_state.get_belief_by_label('workshop'),
        {traits: []}
      );
    ...
    it('learn_about directly from base belief works', () => {
      ...
      const base_hammer = world_mind_state.get_belief_by_label('base_hammer');
    ...
    it('learn_about should dereference trait beliefs to learning mind', () => {
      const world_state = createMindWithBeliefs('world', {
        ...
        hammer: {
          bases: ['PortableObject'],
          traits: { location: 'workshop' }  // String reference to workshop belief
        }
      });
      ...
      const hammer_knowledge = learn_about(npc_mind_state,
        world_state.get_belief_by_label('hammer'),
        {traits: ['location']}
      );

      // Expected behavior: trait references should be dereferenced to npc_mind
      const location_traittype = Traittype.get_by_label('location');
      const location_ref = hammer_knowledge.get_trait(npc_mind_state, location_traittype)?.get_belief_by_state(npc_mind_state);
      ...
      expect(location_ref.get_about(npc_mind_state)).to.equal(world_state.get_belief_by_label('workshop'));
    ...
    it('learn_about should reuse existing beliefs with same about reference', () => {
      ...
      const existing_workshop = npc_mind_state.add_belief_from_template({
                bases: ['Location'],
        traits: {'@about': world_state.get_belief_by_label('workshop').subject}, label: 'my_workshop'
      });
    ...
    it('learn_about should update first belief when multiple beliefs about same entity exist', () => {
      ...
      const belief1 = npc_mind_state.add_belief_from_template({
                bases: ['Location'],
        traits: {'@about': world_state.get_belief_by_label('workshop').subject}, label: 'workshop_belief_1'
      });

      const belief2 = npc_mind_state.add_belief_from_template({
                bases: ['Location'],
        traits: {'@about': world_state.get_belief_by_label('workshop').subject}, label: 'workshop_belief_2'
      });
      ...
      const location_traittype = Traittype.get_by_label('location');
      const location_ref = hammer_knowledge.get_trait(new_state, location_traittype)?.get_belief_by_state(new_state);
    ...
    it('learn_about should copy non-Belief trait values as-is', () => {
      ...
      const color_traittype = Traittype.get_by_label('color');
      expect(hammer_knowledge.get_trait(npc_mind_state, color_traittype)).to.equal('red');
    });
    ...
    it('learn_about should dereference arrays of beliefs', () => {
      ...
      const items_raw = chest_knowledge.get_trait(npc_mind_state, items_traittype);
    ...
    it('learn_about copies trait values even when inherited from base', () => {
      ...
      const world_state2 = world_state.branch(logos().origin_state, 101);
      ...
      const location_traittype = Traittype.get_by_label('location');
      expect(player_hammer._traits.has(location_traittype)).to.be.true;
      const player_knowledge_of_location = player_hammer.get_trait(player_state, location_traittype);
      ...
      const about_subject = location_knowledge_belief.get_trait(player_state, about_traittype);
    ...
    it('incremental knowledge accumulation via inheritance', () => {
      ...
      const color_traittype = Traittype.get_by_label('color');
      const location_traittype = Traittype.get_by_label('location');
      expect(player_hammer_v1._traits.has(color_traittype)).to.be.true;
      expect(player_hammer_v1._traits.has(location_traittype)).to.be.false;
      expect(player_hammer_v1.get_trait(player_state1, color_traittype)).to.equal('grey');
      ...
      expect(player_hammer_v2._traits.has(color_traittype)).to.be.false;  // not in _traits

      // But get_trait finds both
      const player_knowledge_of_location_v2 = player_hammer_v2.get_trait(player_state2, location_traittype);
      ...
      const about_traittype = Traittype.get_by_label('@about');
      expect(location_belief.get_trait(player_state2, about_traittype)).to.equal(workshop.subject);

      expect(player_hammer_v2.get_trait(player_state2, color_traittype)).to.equal('grey');  // inherited!
    });
  });
  ...
  describe('Observable Trait Filtering', () => {
    it('learns only visual traits when modality is visual', () => {
      ...
      const color_traittype = Traittype.get_by_label('color');
      expect(hammer_knowledge._traits.has(color_traittype)).to.be.true;
      expect(hammer_knowledge.get_trait(npc_state, color_traittype)).to.equal('blue');
